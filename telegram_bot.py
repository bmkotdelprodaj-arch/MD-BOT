...
"""
    ...
    def format_report_summary(self, summary):
        ...
        # Выводим данные по сырам
        for cheese in Config.CHEESE_TYPES:
            start = summary['cheese_start'].get(cheese, 0)
            end = summary['cheese_end'].get(cheese, 0)
            sold = summary['cheese_sold'].get(cheese, 0)
            message += f"🧀 {cheese}: начало дня: {start}, конец дня: {end}, продано: {sold}\n"
...
"""
        ...
        # Выводим остатки и продажи по сырам
        for cheese in Config.CHEESE_TYPES:
            data = report['cheese_data'].get(cheese, {'start': 0, 'end': 0, 'sold': 0})
            message += f"🧀 {cheese}: начало дня: {data['start']}, конец дня: {data['end']}, продано: {data['sold']}\n"
