...
class TelegramBot:
    ...
    def send_message(self, text, parse_mode="HTML", reply_markup=None, chat_id=None):
        ...
        # Используем chat_id из аргумента, если передан, иначе из конфига
        ...
        try:
            ...
            logger.info(f"send_message: Message sent successfully to chat_id {target_chat_id}")
            return True
    ...
    def send_start_menu(self, chat_id=None):
...
"""
        ...
        return self.send_message(text, reply_markup=keyboard, chat_id=chat_id)

    def handle_callback(self, callback_data, user_id, google_sheets_service, data_processor):
        """Обрабатывает нажатия инлайн кнопок"""
        try:
            logger.info(f"handle_callback: Processing callback_data '{callback_data}' for user {user_id}")
    ...
    def start_store_history(self, user_id, google_sheets_service):
        ...
        if not dates:
            self.send_message("❌ Нет доступных данных для анализа", chat_id=user_id)
            return
        ...
        self.send_message(text, reply_markup=keyboard, chat_id=user_id)
    ...
    def handle_date_selection(self, user_id, date_str, google_sheets_service, data_processor):
        ...
        try:
            ...
            self.send_message("❌ Неверный формат даты.", chat_id=user_id)
            return
        ...
        if user_type == "store":
            ...
            self.send_message("❌ Неизвестный тип запроса.", chat_id=user_id)
    ...
    def show_network_selection(self, user_id, date_obj, google_sheets_service):
        ...
        try:
            ...
            if not all_networks:
                self.send_message(f"❌ Нет данных по сетям за {date_obj.strftime('%d.%m.%Y')}", chat_id=user_id)
                return
            ...
            self.send_message("❌ Ошибка при формировании списка сетей.", chat_id=user_id)
    ...
    def show_city_selection(self, user_id, date_obj, google_sheets_service):
        ...
        try:
            ...
            if not all_cities:
                self.send_message(f"❌ Нет данных по городам за {date_obj.strftime('%d.%m.%Y')}", chat_id=user_id)
                return
            ...
            logger.error(traceback.format_exc())
            self.send_message("❌ Ошибка при формировании списка городов.", chat_id=user_id)
    ...
    def handle_city_selection(self, user_id, city, google_sheets_service, data_processor):
        ...
        if not date_obj:
            self.send_message("❌ Ошибка: дата не выбрана.", chat_id=user_id)
            return
        ...
        try:
            ...
            if not city_reports:
                self.send_message(f"❌ Нет данных по городу {city} за {date_obj.strftime('%d.%m.%Y')}", chat_id=user_id)
                return
            ...
            self.send_message(message, chat_id=user_id)

        except Exception as e:
            logger.error(f"handle_city_selection failed: {e}")
            logger.error(traceback.format_exc())
            self.send_message("❌ Ошибка при формировании отчета по городу.", chat_id=user_id)
        ...
        try:
            ...
            if user_type == "store":
                ...
                if not network_reports:
                    self.send_message(f"❌ Нет данных по сети {network} за {date_obj.strftime('%d.%m.%Y')}", chat_id=user_id)
                    return
                ...
                if not cities:
                    self.send_message(f"❌ Нет данных по городам в сети {network} за {date_obj.strftime('%d.%m.%Y')}", chat_id=user_id)
                    return
            ...
            logger.error(traceback.format_exc())
            self.send_message("❌ Ошибка при формировании отчета по сети.", chat_id=user_id)
    ...
    def handle_address_selection(self, user_id, address, google_sheets_service, data_processor):
        ...
        if not all([date_obj, network, city]):
            self.send_message("❌ Ошибка: не все параметры выбраны.", chat_id=user_id)
            return
        ...
        try:
            ...
            if not found_report:
                self.send_message(f"❌ Отчет по адресу '{address}' не найден.", chat_id=user_id)
                return
            ...
            logger.error(traceback.format_exc())
            self.send_message("❌ Ошибка при формировании отчета по магазину.", chat_id=user_id)
    ...
    def show_general_date_stats(self, user_id, date_obj, google_sheets_service, data_processor):
        ...
        try:
            ...
            if morning_filtered.empty and evening_filtered.empty:
                self.send_message(f"❌ Нет данных за {date_obj.strftime('%d.%m.%Y')}", chat_id=user_id)
                return
            ...
            logger.error(traceback.format_exc())
            self.send_message("❌ Ошибка при формировании отчета.", chat_id=user_id)
